...
public final class DungeonTimeParser {
  ...
  public static Duration parseDuration(@NotNull String string) {
    ...
    if (tokens.size() < 2) {
      throw new IllegalArgumentException("řetězec by měl poskytovat alespoň jeden pár multiplikátorů.");
    }
    long milliseconds = 0;
    ...
    for (String token : tokens) {
      if (multiplier == null) {
        try {
          ...
          if (multiplier <= 0) {
            throw new InvalidMultiplierException("nejsou povoleny multiplikátory.");
          }
        } catch (NumberFormatException bad) {
          throw new InvalidMultiplierException(token + " není platný multiplikátor.");
        }
      } else {
  ...
  private static DungeonTimeUnit parseDungeonTimeUnit(final String token) {
    ...
    try {
      ...
      throw new InvalidUnitException(token + " není platná jednotka.");
    }
  }

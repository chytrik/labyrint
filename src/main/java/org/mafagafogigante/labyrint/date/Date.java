package org.mafagafogigante.labyrint.date;
...
public class Date implements Comparable<Date>, Serializable {
  ...
  public Date(long year, long month, long day, long hour, long minute, long second) {
    this(year, month, day);
    if (hour < 0) {
      DungeonLogger.warning("Pokus vytvořit Datum s negativní hodinou.");
      hour = 0;
    } else if (hour >= DungeonTimeUnit.DAY.as(DungeonTimeUnit.HOUR)) {
      DungeonLogger.warning("Pokus vytvořit Datum s neexistující hodinou.");
    ...
    if (minute < 0) {
      DungeonLogger.warning("Pokus vytvořit Datum s negativní minutou.");
      minute = 0;
    } else if (minute >= DungeonTimeUnit.HOUR.as(DungeonTimeUnit.MINUTE)) {
      DungeonLogger.warning("Pokus vytvořit Datum s neexistující minutou.");
    ...
    if (second < 0) {
      DungeonLogger.warning("Pokus vytvořit Datum s negativní vteřinou.");
      second = 0;
    } else if (second >= DungeonTimeUnit.MINUTE.as(DungeonTimeUnit.SECOND)) {
      DungeonLogger.warning("Pokus vytvořit Datum s neexistující vteřinou.");
  ...
  public Date(long year, long month, long day) {
    if (year <= 0) {
      DungeonLogger.warning("Pokus vytvořit Datum s negativním rokem.");
      year = 1;
    }
    if (month <= 0) {
      DungeonLogger.warning("Pokus vytvořit Datum s negativním měsícem.");
      month = 1;
    } else if (month > DungeonTimeUnit.YEAR.as(DungeonTimeUnit.MONTH)) {
      DungeonLogger.warning("Pokus vytvořit Datum s neexistujícím měsícem.");
      month = DungeonTimeUnit.YEAR.as(DungeonTimeUnit.MONTH);
    }
    if (day <= 0) {
      DungeonLogger.warning("Pokus vytvořit Datum s negativním dnem.");
      day = 1;
    } else if (day > DungeonTimeUnit.MONTH.as(DungeonTimeUnit.DAY)) {
      DungeonLogger.warning("Pokus vytvořit Datum s neexistujícím dnem.");
  ...
  public Date plus(long amount, DungeonTimeUnit unit) {
    if (amount <= 0) {
      ...
      throw new IllegalArgumentException("jednotka by neměla být nulová.");
    } else {
  ...
  public String toDateString() {
    return "den " + getDay() + " měsíce " + getMonth() + " roku " + getYear();
  }

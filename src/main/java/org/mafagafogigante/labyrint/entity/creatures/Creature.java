...
import org.mafagafogigante.labyrint.entity.LightSource;
import org.mafagafogigante.labyrint.entity.Luminosity;
import org.mafagafogigante.labyrint.entity.TagSet;
import org.mafagafogigante.labyrint.entity.items.CreatureInventory;
...
public class Creature extends Entity {
  ...
  public void setWeapon(Item weapon) {
    if (inventory.hasItem(weapon)) {
      if (weapon.hasTag(Item.Tag.WEAPON)) {
        ...
        DungeonLogger.warning(String.format("Pokusili jsme se nasadit %s (bez značky ZBRAŇ) na %s.",
                weapon.getName(), getName()));
      }
    } else {
      DungeonLogger.warning("Pokusil se nasadit věc, která není v inventáři " + getName() + ".");
    }
  }
  ...
  void dropItem(Item item) {
    if (getInventory().hasItem(item)) {
      ...
      throw new IllegalStateException("věc by měla být v inventáři bytosti.");
    }
  }
  ...
  public void setCauseOfDeath(@NotNull CauseOfDeath causeOfDeath) {
    if (this.causeOfDeath != null) {
      throw new IllegalStateException("tvor již má CauseOfDeath.");
    } else if (getHealth().isAlive()) {
      throw new IllegalStateException("tvor je stále naživu.");
    } else {

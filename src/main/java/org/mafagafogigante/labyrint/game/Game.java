...
import org.mafagafogigante.labyrint.commands.IssuedCommand;
import org.mafagafogigante.labyrint.commands.IssuedCommandEvaluation;
import org.mafagafogigante.labyrint.commands.IssuedCommandProcessor;
...
public class Game {
  ...
  private static void suggestTutorial() {
    ...
    string.append("\nMožná se budete chtít podívat na výuku, abyste pochopili pár základních informací o hře. Výuku vyvoláte příkazem „výuka“.\n");
    Writer.write(string);
  }
  ...
  private static GameState getAfterDeathGameState() {
    ...
    if (gameState != null) {
      JOptionPane.showMessageDialog(getGameWindow(), "Načetla se poslední uložená hra.");
    } else {
      gameState = Loader.newGame();
      JOptionPane.showMessageDialog(getGameWindow(), "Uloženou hru nelze načíst. Vytvořena nová hra.");
    }
    return gameState;
  }
  ...
  private static boolean processInput(IssuedCommand issuedCommand) {
    ...
    if (evaluation.isValid()) {
      ...
      string.append("Zadali jste neplatný příkaz.\n");
      string.append("Ale je to podobné jako ");
      ...
      string.append(".\n");
      string.setColor(Color.ORANGE);
      string.append("Úplný seznam příkazů se vám zobrazí zadáním slova „příkazy“.");
      Writer.write(string);
      return false;
    }
  }

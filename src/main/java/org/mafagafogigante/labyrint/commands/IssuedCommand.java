...
public final class IssuedCommand {
  ...
  public IssuedCommand(@NotNull String source) {
    if (!isValidSource(source)) {
      throw new IllegalArgumentException("neplatný zdroj, porušené příkazové limity.");
    }
    tokens = Collections.unmodifiableList(Arrays.asList(StringUtils.split(source)));
    if (tokens.isEmpty()) {
      throw new IllegalArgumentException("neplatný zdroj, nebyly získány žádné tokeny.");
    }
